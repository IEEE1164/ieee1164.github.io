...
Die Ausgabe der unterstützten Verschlüssel&shy;ungs&shy;verfahren (seit Version v6.3) über: `$ ssh -Q cipher`. Zur Fehlersuche lassen sich detaillierte Informationen ausgeben über: `$ ssh -v`. Aus&shy;gabe der OpenSSH-Version per: `$ ssh -V`.
...
PuTTY ist ein unter Microsoft Windows verbreiteter SSH-Client, es beinhaltet ebenfalls SCP (`pscp.exe`), SFTP (`psftp.exe`) und einen Agent (`pageant.exe`). Mit `plink.exe` steht eine Kommando&shy;zeilenversion des PuTTY-Clients zur Verfügung, es ist somit sowohl als CLI als auch als GUI-Version nutzbar.
