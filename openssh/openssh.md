...
PuTTY ist ein unter Microsoft Windows verbreiteter SSH-Client, es beinhaltet ebenfalls SCP (`pscp.exe`), SFTP (`psftp.exe`) und einen Agent (`pageant`). Mit `plink` steht eine Kommando&shy;zeilenversion des PuTTY-Clients zur Verfügung. 
