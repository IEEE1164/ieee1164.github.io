...
PuTTY ist ein unter Microsoft Windows verbreiteter SSH-Client, es beinhaltet ebenfalls SCP (`pscp.exe`), SFTP (`psftp.exe`) und einen Agent (`pageant.exe`). Mit `plink.exe` steht eine Kommando&shy;zeilenversion des PuTTY-Clients zur Verfügung, es ist somit sowohl von der CLI als auch als GUI nutzbar.
